...
namespace Azul
{
	void ConvertSkin(const char *const pName, const char *const pTargetName, int meshNum)
	{
		assert(pName);
		assert(pTargetName);
		Trace::out("ConvertSkin(%s): ", pName);
		...
		size_t inverseMatrixAccessorIndex = (size_t)gltfModel.skins[meshNum].inverseBindMatrices;
		size_t weightAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].attributes.find("WEIGHTS_0")->second;
		size_t jointAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].attributes.find("JOINTS_0")->second;
		size_t indexAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].indices;
		size_t posAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].attributes.find("POSITION")->second;
		size_t normsAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].attributes.find("NORMAL")->second;
		size_t texCoordAccessorIndex = (size_t)gltfModel.meshes[meshNum].primitives[0].attributes.find("TEXCOORD_0")->second;
	...
	void ConvertInvBind(const char *const pName, const char *const pTargetName, int meshNum)
	{
		assert(pName);
		assert(pTargetName);
		char sBuff[40];
