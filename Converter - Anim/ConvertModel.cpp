...
void ConvertModel(const char *const pFileName, const char *const pTargetName)
{
	...
	GLTF::SetTexture(gltfModel, "Color", texData, pBinaryBuff);
	...
	textureData_proto tA_proto;
	texData.Serialize(tA_proto);
	...
	pProtoFileType = nullptr;
	status = GLTF::GetAzulProtoType(pProtoFileType, texData);
	assert(status);
	...
	//std::string strOut;
	tA_proto.SerializeToString(&strOut);
