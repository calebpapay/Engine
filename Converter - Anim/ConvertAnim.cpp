...
namespace Azul
{
	void ConvertAnim(const char *const pName, const char *const pTargetName, int meshNum)
	{
		assert(pName);
		Trace::out("ConvertAnim(%s):", pName);
		...
		// ANIMATION
		ANIM animData;
		animData.CreateAnim(gltfModel, pTargetName, meshNum);
