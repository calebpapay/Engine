...
namespace Azul
{
	Mesh::Mesh(unsigned int _numVerts, unsigned int _numIndices, unsigned int _numBones)
		:
		numBones(_numBones),
		numVerts{ _numVerts },
		...
		poInvBindArray{ nullptr }
	{
		if(numBones > 0)
		{
			poInvBindArray = new Mat4[numBones]();
	...
	void Mesh::TransferConstantBufferWorldViewProj(Camera *pCam, Mat4 *pWorld)
	{
		...
		DirectXDeviceMan::GetContext()->UpdateSubresource(poConstantBuff_vsView, 0, nullptr, &pCam->getViewMatrix(), 0, 0);
		DirectXDeviceMan::GetContext()->UpdateSubresource(poConstantBuff_vsProjection, 0, nullptr, &pCam->getProjMatrix(), 0, 0);
