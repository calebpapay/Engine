...
namespace Azul
{
	...
	void GameObjectAnimSkin::privWorldComputeExecute()
	{
		...
		// Set the InvBind to Compute shader
		assert(poWorldCompute->poVsInvBind);

		// Set the InvBind to Vertex shader
		assert(poWorldCompute->poVsInvBind);
		poWorldCompute->poVsInvBind->BindCompute(ShaderResourceBufferSlot::InvBind);
		...
		//		HRESULT hr;
		//hr = DirectXDeviceMan::GetContext()->Map(poWorldCompute->poUAVWorldMat->poComputeUAVBuffer,
		//										 0,
		//										 D3D11_MAP_READ,
		//										 0,
		//										 &MappedResource);
